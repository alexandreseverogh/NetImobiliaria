...
DECLARE
    ...
    v_menu JSONB;
BEGIN
    -- 1. Identificar se o usuário é Master Admin
    SELECT EXISTS (
    ...
    WITH permitted_features AS (
        ...
        WHERE sf.is_active = true
          ...
          -- FILTRO B: PROVISÃO (Lógica Híbrida: Módulo + Exceção)
          AND (
            v_is_master = true OR
            p_tenant_id IS NULL OR
            (
                -- Caso 1: Existe uma exceção explícita ATIVA
                EXISTS (
                ...
                OR 
                (
                    -- Caso 2: Pertence a um Módulo Ativo e NÃO há uma exceção explícita INATIVA
                    EXISTS (
                    ...
                    AND NOT EXISTS (
                        SELECT 1 FROM public.tenant_feature_overrides tfo
                        WHERE tfo.feature_id = sf.id AND tfo.tenant_id = p_tenant_id AND tfo.is_active = false
                    )
                )
            )
          )
