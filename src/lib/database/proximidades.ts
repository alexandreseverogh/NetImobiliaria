...
export async function updateImovelProximidades(
  ...
  try {
    console.log('🔍 updateImovelProximidades - Iniciando atualização para imóvel:', imovelId)
    console.log('🔍 updateImovelProximidades - Proximidades recebidas:', proximidades)
    
    // Verificar se todos os proximidade_id existem
    const proximidadeIds = proximidades.map(p => p.proximidade_id)
    console.log('🔍 updateImovelProximidades - IDs de proximidades a verificar:', proximidadeIds)
    
    const checkQuery = `
      SELECT id FROM proximidades WHERE id = ANY($1)
    `
    const checkResult = await client.query(checkQuery, [proximidadeIds])
    console.log('🔍 updateImovelProximidades - IDs encontrados na tabela proximidades:', checkResult.rows.map(r => r.id))
    
    const existingIds = checkResult.rows.map(r => r.id)
    const invalidIds = proximidadeIds.filter(id => !existingIds.includes(id))
    
    if (invalidIds.length > 0) {
      console.error('❌ updateImovelProximidades - IDs inválidos encontrados:', invalidIds)
      throw new Error(`IDs de proximidades inválidos: ${invalidIds.join(', ')}`)
    }
    ...
    if (proximidades.length > 0) {
      ...
      proximidades.forEach(proximidade => {
        const distanciaMetros =
          proximidade.distancia_metros === undefined ||
          proximidade.distancia_metros === null ||
          Number.isNaN(proximidade.distancia_metros)
            ? null
            : proximidade.distancia_metros

        const tempoCaminhada =
          proximidade.tempo_caminhada === undefined || proximidade.tempo_caminhada === null
            ? null
            : typeof proximidade.tempo_caminhada === 'number' && Number.isNaN(proximidade.tempo_caminhada)
              ? null
              : proximidade.tempo_caminhada
