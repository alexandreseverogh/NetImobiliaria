...
BEGIN
    ...
    WITH permitted_features AS (
        ...
        WHERE sf.is_active = true
          -- 1. Filtro de Permissão do Usuário
          AND (
            v_is_master = true OR
            EXISTS (
          ...
          -- 2. FILTRO DE PROVISÃO (Soberania Individual)
          AND (
            v_is_master = true OR
    ...
    category_structure AS (
        ...
        WHERE sc.is_active = true
          ...
          AND (
            v_is_master = true OR
            (sc.module_id IS NOT NULL AND EXISTS (SELECT 1 FROM public.tenant_modules tm WHERE tm.module_id = sc.module_id AND tm.tenant_id = p_tenant_id AND tm.is_enabled = true)) OR
