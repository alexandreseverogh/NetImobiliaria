...
export function useImageUpload(): UseImageUploadReturn {
  ...
  const loadImages = useCallback(async (imovelId: number) => {
    try {
      ...
      console.log('🔍 useImageUpload loadImages - Ordem das imagens:', imagensFormatadas.map((img: any) => ({ id: img.id, ordem: img.ordem })))
  ...
  const deleteImage = useCallback(async (imovelId: number, imagemId: number) => {
    try {
      ...
      setImagens(prev => prev.filter(img => img.id !== imagemId.toString()))
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erro desconhecido')
      throw err
    } finally {
  ...
  const setPrincipalImage = useCallback(async (imovelId: number, imagemId: number) => {
    try {
      ...
      setImagens(prev => prev.map(img => ({
        ...img,
        principal: img.id === imagemId.toString()
      })))
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erro desconhecido')
      throw err
