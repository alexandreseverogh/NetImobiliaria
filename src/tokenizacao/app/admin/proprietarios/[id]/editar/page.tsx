...
export default function EditarProprietarioPage() {
  ...
  useEffect(() => {
    ...
    if (!hasPermission('proprietarios', 'WRITE')) {
      router.push('/admin')
      return
    }
  ...
  useEffect(() => {
    if (proprietario && estadosCidades.estados.length > 0) {
      // Encontrar estado pelo nome
      const estadoEncontrado = estadosCidades.estados.find(e => e.nome === proprietario.estado_fk)
      if (estadoEncontrado) {
        setFormData(prev => ({ ...prev, estado: estadoEncontrado.id }))
      }
    }
  }, [proprietario, estadosCidades.estados])

  useEffect(() => {
    if (proprietario && formData.estado && estadosCidades.municipios.length > 0) {
  ...
  }, [proprietario, formData.estado, estadosCidades.municipios])
  ...
  const checkCPFExists = async (cpf: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/proprietarios/verificar-cpf?cpf=${cpf}&excludeId=${params.id}`)
