...
export default function ProximidadesPage() {
  ...
  const fetchProximidades = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/admin/proximidades?page=${currentPage}&limit=10${categoriaParam}${searchParam}`)
      if (response.ok) {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Tem certeza que deseja excluir esta proximidade?')) {
      try {
        const response = await fetch(`/api/admin/proximidades/${id}`, {
          method: 'DELETE'
        })
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600 mt-2">Gerencie as proximidades disponÃ­veis</p>
            </div>
            <button
