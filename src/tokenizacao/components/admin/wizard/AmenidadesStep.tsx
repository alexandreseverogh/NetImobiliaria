...
export default function AmenidadesStep({ data, onUpdate, mode }: AmenidadesStepProps) {
  ...
  const [selectedAmenidades, setSelectedAmenidades] = useState<number[]>(data.amenidades?.map(a => a.id) || [])
  ...
  const updateParentData = useCallback((newSelectedAmenidades: number[]) => {
    ...
    updateTimeoutRef.current = setTimeout(() => {
      if (amenidades.length > 0 && !loading && !isInitialLoad.current) {
        const selectedAmenidadesData = amenidades.filter(a => newSelectedAmenidades.includes(a.id))
        onUpdate({
          amenidades: selectedAmenidadesData
        })
      }
  ...
  const loadData = async () => {
    try {
      ...
      // Carregar categorias
      const categoriasResponse = await fetch(API_ENDPOINTS.AMENITIES.CATEGORIES)
      ...
      // Carregar amenidades
      const amenidadesResponse = await fetch(API_ENDPOINTS.AMENITIES.LIST)
  ...
  const handleSelectAllInCategory = useCallback((categoriaId: number) => {
    ...
    setSelectedAmenidades(prev => {
      const newSelection = allSelected
        ...
        : [...new Set([...prev, ...amenidadesInCategory])] // Marcar todas
