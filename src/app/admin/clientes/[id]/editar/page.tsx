...
export default function EditarClientePage() {
  ...
  useEffect(() => {
    ...
    const verificarEmail = async () => {
      ...
      try {
        ...
        const response = await post('/api/admin/clientes/verificar-email', { email: emailLimpo, excludeUuid: params.id })
    ...
    return () => clearTimeout(timeoutId)
  }, [formData.email, post, params.id])

  // Encontrar estado e cidade baseado nos dados do cliente
  useEffect(() => {
    if (cliente && estadosCidades.estados.length > 0 && !formData.estado) {
      ...
      if (estadoEncontrado) {
        ...
        console.error('❌ [EDITAR CLIENTE] Estado NÃO encontrado para:', cliente.estado_fk)
      }
    }
  }, [cliente, estadosCidades.estados, formData.estado])
  ...
  useEffect(() => {
    if (cliente && cliente.cidade_fk && formData.estado && estadosCidades.municipios.length > 0 && !formData.cidade) {
      ...
      if (cidadeEncontrada) {
        ...
        console.warn('⚠️ Cidade não encontrada na lista de municípios:', cliente.cidade_fk)
        console.warn('⚠️ Municípios disponíveis:', estadosCidades.municipios.map(m => m.nome).slice(0, 10))
      }
    }
  }, [cliente, formData.estado, estadosCidades.municipios, formData.cidade])
  ...
  const checkCPFExists = async (cpf: string) => {
    ...
    try {
      ...
      const response = await post('/api/admin/clientes/verificar-cpf', { cpf, excludeUuid: params.id })
      const data = await response.json()
