...
export default function AmenidadesPage() {
  ...
  const fetchAmenidades = useCallback(async () => {
    try {
      ...
      // Usando hook centralizado - token adicionado automaticamente!
      const response = await get(`/api/admin/amenidades?page=${currentPage}&limit=10${categoriaParam}${searchParam}`)
  ...
  const handleDelete = async (amenidade: Amenidade) => {
    if (confirm('Tem certeza que deseja excluir esta amenidade?')) {
      try {
        const response = await del(`/api/admin/amenidades/${amenidade.slug}`)
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex space-x-2">
                            ...
                            <DeleteGuard resource="amenidades">
                              <button
                                onClick={() => handleDelete(amenidade)}
