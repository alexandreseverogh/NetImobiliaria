...
export default function TiposImoveisPage() {
  ...
  const fetchTipos = useCallback(async () => {
    try {
      const response = await get('/api/admin/tipos-imoveis');
      if (!response.ok) {
  ...
  const handleCreate = () => {
    ...
    console.log('🔍 TiposImoveisPage: hasPermission WRITE:', hasPermission('tipos-imoveis', 'UPDATE'));
    router.push('/admin/tipos-imoveis/novo');
  };

  const handleToggleStatus = async (id: number, ativo: boolean) => {
    try {
      const response = await patch(`/api/admin/tipos-imoveis/${id}`, { ativo: !ativo });
