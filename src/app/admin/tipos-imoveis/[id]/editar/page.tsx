...
export default function EditarTipoImovelPage() {
  ...
  const fetchTipo = useCallback(async (id: string) => {
    try {
      const response = await get(`/api/admin/tipos-imoveis/${id}`);
      if (!response.ok) {
  ...
  }, [user, router, params.id, fetchTipo]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await put(`/api/admin/tipos-imoveis/${params.id}`, formData);
