...
export async function GET(
  ...
  try {
    ...
    if (action.status !== 'PENDING_APPROVAL') {
      return htmlResponse(
        '⚠️ Status inválido',
        `Esta ação está com status "${action.status}" e não pode ser aprovada agora.`,
        400
      );
    }

    // Busca settings do tenant para obter credenciais Meta (raw SQL — independente do Prisma model)
    let settings: any = null;
    if (action.tenantId) {
      const sRes = await pool.query(
        `SELECT * FROM campanhasmarketingdigital."Settings" WHERE tenant_id = $1::uuid LIMIT 1`,
        [action.tenantId]
      );
      settings = sRes.rows[0] || null;
    }

    // Muda status para PENDING_EXECUTION antes de executar (evita dupla execução)
    ...
    console.error('GET /api/agent/approve error:', error);
    return htmlResponse('❌ Erro interno', `Não foi possível executar a ação: ${error.message}`, 500);
  }
}
