...
export async function GET(request: NextRequest) {
  try {
    ...
    const tenantId = currentUser?.is_system_role ? undefined : currentUser?.tenantId
  ...
  try {
    ...
    try {
      ...
      for (const estado of estados) {
        ...
        await pool.query(
          `INSERT INTO valor_destaque_local (tenant_id, estado_fk, cidade_fk, valor_destaque, created_at, updated_at)
           VALUES ($1, $2, 'TODAS', $3, NOW(), NOW())
           ON CONFLICT (tenant_id, estado_fk, cidade_fk) 
           DO UPDATE SET valor_destaque = $3, updated_at = NOW()`,
          [tenantId, sigla.toUpperCase(), valorDestaqueNumerico]
        )
