...
import { getTenantFilterFromRequest, buildTenantWhereClause } from '@/lib/auth/get-tenant-from-token';

// GET /api/admin/logs - Listar logs de login/logout (com isolamento por tenant)
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      // Filtro de tenant (null = master vê tudo; string = admin vê só o seu)
      const tenantFilter = buildTenantWhereClause(tenantId, '', paramIndex);
      ...
      // Estatísticas (respeitando filtro de tenant)
      const statsQuery = `
        SELECT
          COUNT(*) as total_logs,
      ...
      // Total de usuários cadastrados: filtrado por tenant ou global (master)
      const totalUsersQuery = tenantId
        ? `SELECT COUNT(*) as total_users
           FROM users u
           JOIN user_tenant_membership utm ON u.id = utm.user_id
           WHERE u.ativo = true AND utm.tenant_id = $1 AND utm.is_active = true`
        : `SELECT COUNT(*) as total_users FROM users WHERE ativo = true`;

      const totalUsersResult = tenantId
        ? await client.query(totalUsersQuery, [tenantId])
        : await client.query(totalUsersQuery);
