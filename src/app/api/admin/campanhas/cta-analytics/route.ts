 ...
 *
 * Query params: from, to (ISO date) · segmentId (<uuid>) · clientId ('own'|<uuid>|'all') ·
...
export async function GET(request: NextRequest) {
  ...
  // Builder de filtro — segmentId tem precedência sobre clientId
  function filt(f: Date, t: Date) {
    ...
    if (segmentId) {
      args.push(segmentId)
      where.push(`client_id IN (SELECT uuid FROM public.clientes WHERE segment_id = $${args.length} AND tenant_id = $1)`)
  ...
  const [
    ...
    viewsDay, subsDay, viewsByDest, subsByDest, destNames, byCta, byOrigem, heat, leadsPerHour, segments,
  ] = await Promise.all([
    pool.query(`SELECT
        ...
        -- Submits do funil real: apenas destinos que também tiveram VIEW no mesmo período
        COUNT(*) FILTER (WHERE event_type='SUBMIT' AND destination_id IN (
          SELECT DISTINCT destination_id FROM ${S}."CtaInteraction"
          WHERE event_type='VIEW' AND tenant_id=$1 AND created_at >= $2 AND created_at < $3
    ...
    pool.query(`SELECT EXTRACT(DOW FROM created_at AT TIME ZONE 'America/Sao_Paulo')::int AS dow,
               EXTRACT(HOUR FROM created_at AT TIME ZONE 'America/Sao_Paulo')::int AS hr, COUNT(*) AS n
      FROM ${S}."CtaSubmission" WHERE ${cur.w} GROUP BY 1,2`, cur.args),
    pool.query(`SELECT EXTRACT(HOUR FROM created_at AT TIME ZONE 'America/Sao_Paulo')::int AS hr, COUNT(*) AS n
      FROM ${S}."CtaSubmission" WHERE ${cur.w} AND lead_uuid IS NOT NULL GROUP BY 1 ORDER BY 1`, cur.args),

    // Segmentos com clientes ativos para este tenant
    pool.query(`
      SELECT ss.id, ss.name, ss.color_theme, COUNT(c.uuid)::int AS clients_count
      FROM public.system_segments ss
      INNER JOIN public.clientes c ON c.segment_id = ss.id AND c.tenant_id = $1
      WHERE ss.is_active = true
      GROUP BY ss.id, ss.name, ss.color_theme
      ORDER BY ss.name
    `, [payload!.tenantId]),
  ])
  ...
  const funnelSubmits = num(kpiInt.rows[0]?.funnel_submits) // SUBMIT pareados com VIEW (excl. webhook puro)
  const submits       = num(kpiSub.rows[0]?.submits)        // total incluindo webhook
  ...
  const leadsHourly  = Array.from({ length: 24 }, (_, hr) => ({
    hr,
    label: `${String(hr).padStart(2, '0')}:00h – ${String(hr).padStart(2, '0')}:59h`,
    n: num(leadsPerHour.rows.find((r: any) => Number(r.hr) === hr)?.n),
  }))
  ...
  return NextResponse.json({
    ...
    kpis: {
      ...
      conversion: views > 0 ? +((funnelSubmits / views) * 100).toFixed(1) : 0,
    ...
    funnel: [
      ...
      { stage: 'Submissões (funil)', value: funnelSubmits },
      { stage: 'Leads no CRM',   value: leads },
    ],
    segments: segments.rows,
    trend, ranking, porCta, porOrigem, heatmap, leadsHourly, insights,
  })
}
