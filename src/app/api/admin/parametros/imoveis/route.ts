...
export async function GET(request: NextRequest) {
  try {
    ...
    const tenantId = currentUser?.tenantId || '00000000-0000-0000-0000-000000000001'
    ...
    const result = await pool.query(
      ...
      FROM parametros_imoveis WHERE tenant_id = $1 LIMIT 1`,
      [tenantId]
    )
    ...
    if (result.rows.length === 0) {
      ...
      await pool.query(
        `INSERT INTO parametros_imoveis (
          tenant_id,
          cobranca_corretor_externo,
          vl_destaque_nacional, 
          valor_corretor, 
          ...
          proximos_corretores_recebem_leads_internos,
          sla_minutos_aceite_lead_interno
        ) VALUES ($1, false, 0.00, 0.00, $2, $3, 0.00, 5, 30, 3, 5, 2000000.00, 3, 15)`,
        [tenantId, '', 'BRASILIA']
      )
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const existe = await pool.query('SELECT 1 FROM parametros_imoveis WHERE tenant_id = $1 LIMIT 1', [tenantId])
    ...
    if (existe.rows.length === 0) {
      ...
      await pool.query(
        `INSERT INTO parametros_imoveis (${columns}) VALUES (${placeholders})`,
        values
      )
    } else {
      // Atualizar valor existente
      await pool.query(
        `UPDATE parametros_imoveis SET ${updates.join(', ')} WHERE tenant_id = $${paramCount}`,
        [...values, tenantId]
      )
    }
