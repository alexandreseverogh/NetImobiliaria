...
export async function GET(request: NextRequest) {
  try {
    // 1. Verificar permissões usando sistema unificado
    const permissionCheck = await unifiedPermissionMiddleware(request)
    if (permissionCheck) return permissionCheck

    // 2. Extrair tenantId do token para isolamento automático
    const token = getTokenFromRequest(request)
    const decoded = token ? await verifyToken(token) : null
    
    // VISÃO MASTER: Se for System Admin (Master), pode ver estatísticas globais se não passar tenantId na query
    const isMasterAdmin = (decoded as any)?.is_system_role === true
    const { searchParams } = new URL(request.url)
    const queryTenantId = searchParams.get('tenantId')
    
    const tenantId = isMasterAdmin ? (queryTenantId || undefined) : (decoded as any)?.tenantId
    ...
    const statsResult = await pool.query(statsQuery, tenantParams)
    ...
    const tiposQuery = `
      ...
      WHERE i.ativo = true ${tenantFilter.replace('tenant_id', 'i.tenant_id')}
      GROUP BY ti.nome
      ORDER BY quantidade DESC
    `
    
    const tiposResult = await pool.query(tiposQuery, tenantParams)
    ...
    const finalidadesResult = await pool.query(finalidadesQuery, tenantParams)
    ...
    const statusResult = await pool.query(statusQuery, tenantParams)
    ...
    const estadosQuery = `
      SELECT 
        ...
        COUNT(*) as quantidade
      FROM imoveis
      WHERE ativo = true ${tenantFilter}
      GROUP BY estado
      ORDER BY quantidade DESC
      LIMIT 10
    `
    
    const estadosResult = await pool.query(estadosQuery, tenantParams)
    ...
    const municipiosResult = await pool.query(municipiosQuery, tenantParams)
    ...
    const evolucaoQuery = `
      ...
      AND ativo = true ${tenantFilter}
      GROUP BY DATE_TRUNC('month', created_at)
      ORDER BY mes
    `
    
    const evolucaoResult = await pool.query(evolucaoQuery, tenantParams)
