...
Write-Host " TESTE AUTOMATIZADO - PROPRIETÁRIOS (UUID ONLY)" -ForegroundColor Cyan
Write-Host "===================================" -ForegroundColor Cyan
...
function Build-ProprietarioBody {
    param(
        [object]$Data,
        [string]$Nome,
        [string]$Telefone,
...
# 3. PUT por UUID (com rollback opcional)
$randomSuffix = Get-Random -Minimum 1000 -Maximum 9999
$testEmail = "uuid-only-$randomSuffix@teste.com"
$testTelefone = "8199$randomSuffix$randomSuffix"
$testNome = "$($snapshot.nome) - UUID"
$body = Build-ProprietarioBody -Data $snapshot -Nome $testNome -Telefone $testTelefone -Email $testEmail
...
if (-not $SkipRevert) {
    ...
    $originalBody = Build-ProprietarioBody -Data $snapshot -Nome $snapshot.nome -Telefone $snapshot.telefone -Email $snapshot.email
    Invoke-Api -Method PUT -Uri $uriUuid -Body $originalBody | Out-Null
}

# 4. Consulta rápida de imóveis (quando solicitado)
if ($ImovelId -gt 0) {
Invoke-PSQL -Command ("SELECT id, proprietario_uuid FROM imoveis WHERE id = $ImovelId;")
}

Write-Host ""
Write-Host "[OK] Testes UUID-only concluídos." -ForegroundColor Green
Write-Host ""
Write-Host "[INFO] Próximos passos recomendados:" -ForegroundColor Cyan
Write-Host " - Executar testes da UI (listagem, edição e deleção) validando uso de UUID."
Write-Host " - Consultar auditoria para confirmar registro com resourceId = UUID."
Write-Host ""
