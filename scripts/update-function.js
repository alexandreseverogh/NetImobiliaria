...
async function updateFunction() {
 ...
 AS $function$
  ...
  try {
    ...
    // Test the output for admmd
    const userRes = await pool.query("SELECT * FROM users WHERE username = 'admmd'");
    const user = userRes.rows[0];
    const tenantRes = await pool.query("SELECT tenant_id FROM user_tenant_membership WHERE user_id = $1", [user.id]);
    const testMenu = await pool.query("SELECT get_sidebar_menu_for_user($1::uuid, 'admin', $2::uuid) AS menu", [user.id, tenantRes.rows[0].tenant_id]);
    console.log("Result for admmd:", JSON.stringify(testMenu.rows[0].menu, null, 2));
