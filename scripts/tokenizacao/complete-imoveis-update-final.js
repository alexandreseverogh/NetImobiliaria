...
async function completeImoveisUpdate() {
  try {
    console.log('🔄 Completando atualização da tabela imoveis - Versão final...\n');
    
    // 1. Remover todas as views que dependem da tabela imoveis
    console.log('1. Removendo views dependentes...');
    const views = [
      'imoveis_completos',
      'imoveis_amenidades_completas', 
      'imoveis_proximidades_completas',
      'amenidades_completas',
    ...
    for (const view of views) {
      try {
        ...
        console.log(`✅ View ${view} removida`);
      } catch (error) {
        console.log(`⚠️  Erro ao remover view ${view}:`, error.message);
      }
    }
    
    // 2. Alterar coluna status_fk de VARCHAR para INTEGER
    console.log('\n2. Alterando coluna status_fk para INTEGER...');
    try {
    ...
    // 3. Alterar tipo das colunas estado_fk e cidade_fk para VARCHAR
    console.log('\n3. Alterando tipo das colunas estado_fk e cidade_fk...');
    ...
    // 4. Remover constraints de chave estrangeira das colunas estado_fk e cidade_fk
    console.log('\n4. Removendo constraints de chave estrangeira...');
    ...
    // 5. Adicionar constraint de chave estrangeira para status_fk
    console.log('\n5. Adicionando constraint para status_fk...');
    try {
      await pool.query(`
        ALTER TABLE imoveis 
    ...
    // 6. Ajustar precisão da coluna taxa_extra para NUMERIC(8,2)
    console.log('\n6. Ajustando precisão da coluna taxa_extra...');
    try {
      await pool.query(`
        ALTER TABLE imoveis 
    ...
    console.log('\n7. Criando índices para as novas colunas...');
    ...
    // 8. Recriar a view imoveis_completos
    console.log('\n8. Recriando view imoveis_completos...');
    try {
      await pool.query(`
    ...
    console.log('\n9. Verificando estrutura final da tabela imoveis...');
