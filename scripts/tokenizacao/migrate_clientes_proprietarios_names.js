...
async function migrateData() {
  try {
    ...
    const clientesResult = await pool.query('SELECT id, estado_fk, cidade_fk FROM clientes WHERE estado_nome IS NULL OR cidade_nome IS NULL')
    ...
    for (const cliente of clientesResult.rows) {
      ...
      await pool.query(
        ...
        [estadoNome, cidadeNome, cliente.id]
      )
    }
    ...
    const proprietariosResult = await pool.query('SELECT id, estado_fk, cidade_fk FROM proprietarios WHERE estado_nome IS NULL OR cidade_nome IS NULL')
    ...
    for (const proprietario of proprietariosResult.rows) {
      ...
      await pool.query(
        ...
        [estadoNome, cidadeNome, proprietario.id]
      )
    }
