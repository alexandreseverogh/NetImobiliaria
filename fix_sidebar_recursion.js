...
BEGIN
    ...
    -- [2] Gerar Menu Holístico com IDs Únicos (Prefixados) para evitar loop de recursão
    ...
    FROM (
        ...
        menu_leafs AS (
            ...
            -- Itens de sistema (Provisionamento Master)
            SELECT 
                s.id as menu_item_id,
                999 as parent_category_id,
                'Master Governance' as group_name,
                s.name as display_name,
                s.url as path,
                s.icon_name as icon,
                s.order_index,
            ...
            WHERE s.is_active = true 
              ...
              AND _v_is_global_master = true
        ),
        menu_final AS (
            -- Pais: Categoria prefixada com 'cat_'
            SELECT DISTINCT ON (parent_category_id)
                'cat_' || parent_category_id as id,
                NULL as parent_id,
                group_name as name,
                NULL as path,
                cat.icon as icon,
            ...
            FROM menu_leafs ml
            LEFT JOIN public.system_categorias cat ON ml.parent_category_id = cat.id
            
            UNION ALL
            
            -- Filhos: Item prefixado com 'item_' e parent apontando para 'cat_'
            SELECT 
                'item_' || menu_item_id as id,
                'cat_' || parent_category_id as parent_id,
...
`;
  ...
  try {
    ...
    console.log('Sidebar corrigida com IDs únicos! Loop de recursão eliminado.');
  } catch (err) {
    console.error(err);
  } finally {
    await client.end();
