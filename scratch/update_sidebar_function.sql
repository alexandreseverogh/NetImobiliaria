CREATE OR REPLACE FUNCTION public.get_sidebar_menu_for_user(p_user_id uuid, p_tenant_id uuid, p_system_id character varying DEFAULT 'admin'::character varying)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
AS $function$
        ...
        authorized_items AS (
            ...
            WHERE s.is_active = true
              ...
              AND (
                _v_is_superadm = true
                OR (
                    -- VERIFICACAO DE ENTITLEMENT (ASSINATURA DO MODULO - HERANCA POR CATEGORIA)
                    EXISTS (
                        SELECT 1
                        FROM public.system_features f
                        LEFT JOIN public.system_feature_modules fm ON f.id = fm.feature_id
                        LEFT JOIN public.system_categorias sc ON f.category_id = sc.id
                        JOIN public.tenant_modules tm ON (fm.module_id = tm.module_id OR sc.module_id = tm.module_id)
                        WHERE f.slug = s.permission_required
                          AND tm.tenant_id = p_tenant_id
                          AND tm.is_enabled = true
                    )
                    AND (
                        -- VERIFICACAO DE PERMISSAO (PODE ACESSAR A FUNCIONALIDADE?)
