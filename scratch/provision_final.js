...
async function run() {
  try {
    console.log('1. Adding unique constraint to parametros.tenant_id...');
    try {
      await pool.query("ALTER TABLE parametros ADD CONSTRAINT parametros_tenant_id_unique UNIQUE (tenant_id)");
    } catch (e) {
      console.log('Constraint might already exist or failed:', e.message);
    }

    console.log('2. Backfilling imoveis with NULL tenant_id to system tenant...');
    await pool.query("UPDATE imoveis SET tenant_id = $1 WHERE tenant_id IS NULL", [systemTenant]);

    console.log(`3. Provisioning parameters for tenant ${tenantId}...`);
    const checkParams = await pool.query("SELECT 1 FROM parametros WHERE tenant_id = $1", [tenantId]);
    if (checkParams.rows.length === 0) {
      await pool.query(`
    ...
    console.log(`4. Provisioning valor_destaque_local for tenant ${tenantId}...`);
    const checkVals = await pool.query("SELECT 1 FROM valor_destaque_local WHERE tenant_id = $1 LIMIT 1", [tenantId]);
    if (checkVals.rows.length === 0) {
      await pool.query(`
    ...
    console.log('5. Moving 5 properties to the user tenant and enabling highlights for testing...');
