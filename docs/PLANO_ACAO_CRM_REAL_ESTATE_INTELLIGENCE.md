...
**Regras de Hierarquia de Distribuição (Fallback):**
1. **Dono do Imóvel:** Se o lead vem de um imóvel com `corretor_fk` preenchido, ele recebe o lead **automaticamente e em caráter definitivo** (sem expiração de SLA).
2. **Meritocracia Regional:** Se o imóvel não tem dono ou se o lead for genérico (IA), entra no pool regional (Internos vs Externos). 
   - A quantidade de tentativas e o tempo de resposta são regidos pelos campos:
...
3. **Corretor Plantonista:** Caso todas as tentativas regionais expirem ou não existam candidatos qualificados na região, o lead é escalado para o **Corretor Plantonista** (atendimento imediato e definitivo).

**Regras Operativas:**
- Ao atribuir lead por Meritocracia, o sistema define `atribuicao_expira_em`.
- Se o corretor não aceitar dentro do prazo (SLA):
  - O job de transbordo (`cron/transbordo`) executa, registra a falha no histórico e solicita um novo candidato à engine, excluindo todos os que já falharam anteriormente para este lead.
- Atribuições via **Dono do Imóvel** ou **Plantonista** possuem `atribuicao_expira_em = NULL` (não transbordam).
...
## 12. M�DULO BI: CYCLE TIME E GARGALOS (EVENT SOURCING)

Como estabelecido em 02/04/2026, foi homologado o plano de a��o de arquitetura de BI para m�tricas de tempo de funil (Cycle Time).

### 12.1. Arquitetura Proposta: Tabela de Hist�rico (Trigger-based)
...
- **Automa��o de Bancos (Database level)**
  - Cria��o de uma PostgreSQL Trigger em `leads_kanban` no evento de `UPDATE`.
  - A trigger fecha a coluna antiga preenchendo `data_saida` e abre uma linha preenchendo `data_entrada` na nova coluna.

### 12.2. Visualiza��es no Dashboard (Kanban Analytics)
- **Mapa de Calor de Gargalos:** Exibi��o da coluna que concentra o maior TTL (Time-to-Live) da rede.
- **Timeline Individual Modal:** Cada modal de lead ter� seu rastreamento cronol�gico.
- **Efici�ncia de SLA do Corretor:** Compara��o de velocity entre membros da roleta.

> *Status: Homologado pelo Comercial. Execu��o Iniciada na Sidebar.*


---

## 13. M�DULO BI: GEST�O DE INVESTIMENTO (CRUD MARKETING & ROI ESTRAT�GICO)

*Aprovado em 02/04/2026. A vis�o estrat�gica foca em fornecer intelig�ncia n�o apenas de Vendas, mas do Controle Qualitativo de M�dia para elevar a Diretoria de Marketing ao topo do mercado imobili�rio nacional.*

### 13.1. Arquitetura Inteligente do Banco de Dados
Cria��o de uma nova tabela aut�noma via Migration (marketing_campanhas_orcamento) para hospedar os inputs das verbas estrat�gicas. Campos obrigat�rios definidos pelo comercial:
- `id` (PK UUID)
- `utm_campaign`: A Chave-Mestra que vai espelhar e cruzar com os dados de capta��o (ex: `Lancamento_Primavera`)
- `plataforma`: Meta Ads, Google Search, TikTok, etc.
- `publico_alvo`: N�vel de segmenta��o sem�ntico (ex: "M�dicos", "Investidores Sul", "Fam�lias MCMV").
- `periodo_faixa_horaria`: (Array/JSONB) Controlando a concentra��o de queima (ex: seg-sex 18h-00h).
- `data_inicio` e `data_fim`: Vig�ncia da queima or�ament�ria.
- `valor_investido_brl`: O custo empenhado pela diretoria.

### 13.2. A Din�mica UX/UI Premium (Como a tela vai se comportar)
A interface precisar� gritar "Premium". O layout abordar� vidros (Glassmorphism), acentos em Blue-Laser e tipografia brutalista estruturada.
- **�rea 1 (Inser��o/CRUD):** Bot�o discreto no topo direito "+ Alocar Verba". Abre um Modal limpo e linear em etapas (Wizard) onde o especialista de m�dia preenche o custo e os recortes demogr�ficos da campanha X.
- **�rea 2 (HUD Estrat�gico - O Drill-down de Elite):**
   - **Card CPL vs P�blico (Score Sem�ntico):** Cruza o `publico_alvo` declarado com a Nota da IA. *Ex: Retorna graficamente que o P�blico "Advogados" custou R/lead, mas 80% estava com "Prontid�o Fria", enquanto "M�dicos" estava em R/lead mas fechou em 3 dias.*
   - **Sankey de Sangria:** Visual progressivo que espelha o gargalo de leads gerados especificamente por uma �nica UTM Campaign at� chegar na coluna de venda. Onde quebrou a campanha? Na convers�o pro chat ou no atendimento?
   - **Dashboard Individual (Drill-Down Matrix):** Se a diretoria clica no bloco da campanha "Investidores Sul", ela � projetada para uma sub-p�gina focada que elenca e ranqueia *Quais Corretores (Com fotos)* performaram melhor especificamente em atender este p�blico.

### 13.3. Matem�tica Parametrizada do ROI Real
O Backend efetuar� um JOIN di�rio entre `marketing_campanhas_orcamento` e `marketing_eventos` (que fica atrelada ao Kanban).
- Ele divide os leads rastreados com UTM pela verba declarada naquela mesma UTM.
- Dispara relat�rios anal�ticos: CPL (Custo por Lead), CPV (Custo por Visita Agendada), CPA (Custo por Agendamento) e CAC (Custo de Venda).


---

## 14. GUARDIAN RULES: PADR�ES DE UX GLOBAIS

**Regra 001 - M�scara Monet�ria Obrigat�ria:** Em QUALQUER formul�rio (CRUD) desenvolvido nesta aplica��o, sempre que existir a requisi��o de input de um "Campo de Valor/Moeda", o componente DEVER� incorporar a m�scara din�mica de formata��o de moeda brasileira (BRL), impedindo digita��o de texto bruto e garantindo o formato `R$ 0.000,00` direto na interface para escalada nativa da experi�ncia Premium.
