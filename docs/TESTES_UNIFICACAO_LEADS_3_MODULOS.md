...
> **Status:** ✅ execução concluída (2026-07-21) — 20/20 cenários passaram. 1 bug real encontrado
> e corrigido (`trg_log_kanban_ciclos` crashava sem `kanban_colunas` ativa — T3); 2 achados de
> produto registrados, não implementados (T3: lead criado via infra compartilhada mesmo só com
> Mensageria contratada; DG2: falta aviso de UX sobre cliques de WhatsApp sem Mensageria).
...
| DG1 | ✅ passou | Confirmado como parte do T1 — sidebar real do tenant-bancada só com `trafego-pago` mostrou exclusivamente a categoria "Campanhas de Marketing Digital", sem nenhuma feature de `crm`. | — |
| DG2 | ✅ passou (achado de UX registrado) | Mesmo cenário do T5, já confirmado (clique isolado sem Mensageria não cria lead, nada quebra). **Achado adicional (grep no código, não implementado):** não existe hoje nenhum aviso explícito na UI de Campanhas (`dashboard`/`leads`/`mecanismos`) avisando que cliques de WhatsApp sem o módulo Mensageria contratado não viram lead identificado — gap de UX real, registrado como pendência de produto, não corrigido nesta rodada (decisão de copy/posicionamento fica pra quando houver prioridade). | — |
| DG3 | ✅ passou | Confirmado como parte do T2 — funil/leads funcionaram plenamente (Concierge IA + DistributionEngine + Kanban), `marketing_eventos` gravado com `campaign_id NULL` sem nenhum erro de query. | — |
| DG4 | ✅ passou (achado real sobre o mecanismo de desprovisionamento) | Tenant-bancada C+R+M, lead completo criado. **Achado real:** desligar só `tenant_modules.is_enabled=false` do módulo `crm` **não** escondeu a categoria CRM da sidebar — lendo `get_sidebar_menu_for_user` inteira, existe uma "NOVA REGRA" (fallback documentado no próprio código da função) que mostra a categoria se a feature tiver `tenant_feature_overrides.is_active=true`, **independente** de `tenant_modules`. Isso não é bug: é o mesmo modelo do `ACCESS_CONTROL.md` — `tenant_feature_overrides` é o ato deliberado de provisionamento (o "contrato"), `tenant_modules` é sinal comercial complementar. Repeti o teste desligando `tenant_feature_overrides` (o mecanismo real que `/admin/master/provisioning` usa) → sidebar corretamente ficou sem nenhuma feature de CRM, e `leads_staging`/`leads_kanban` do lead continuaram 100% intactos. | — |
| I4 | ✅ passou | Reativado `tenant_feature_overrides` das mesmas 12 features de CRM → sidebar voltou a mostrar CRM normalmente, `leads_staging` continua com **exatamente 1** linha pro mesmo email (sem duplicar). | — |
...
| I2 | ✅ passou | Marketing Digital, escopo `clientId=own`. `GET /api/admin/campanhas/leads?clientId=own&limit=1` retornou `total:7`; `SELECT COUNT(*) FROM leads_staging WHERE tenant_id=... AND client_id IS NULL` no mesmo escopo retornou `7` — bate exato. | — |
| I3 | ✅ passou | Inspeção de código (não runtime): `StageFunnelWidget.tsx` mostra "Cliques" (`stage.clicks`, métrica real de anúncio) e "Leads" (`stage.leads`, `CtaInteraction` filtrado por `WHATSAPP_CLICK`) como 2 labels distintos no mesmo card de funil — nunca a mesma palavra pros dois números. `DashboardHelpModal.tsx` documenta explicitamente a diferença ("2️⃣ Cliques... quem achou relevante clicar" vs "4️⃣ Leads... quem deixou os dados de contato"). Nota: o backend (`dashboard/full/route.ts`) nomeia a variável de contagem de `WHATSAPP_CLICK` como `currentLeadCount`/`leads` — é a filosofia deliberada do D1 ("1 lead = 1 clique com interesse real"), documentada no código; não conflita com o requisito de I3 porque a UI nunca rotula um clique genérico (REDIRECT/SUBMIT/VIEW) com a mesma palavra "Leads". | — |
...
| A3 | ✅ passou | **Escopo ajustado:** usado `POST /api/crm/leads` (fonte única real, não o endpoint legado `imovel_prospects`/`imovel_prospect_atribuicoes`, que é um caminho paralelo específico de imóvel, fora do escopo da unificação) — Marketing Digital, sem `campaign_id`/`utm_source`/`ref` nenhum. Resultado: **zero** linhas em `marketing_eventos` (nem sequer uma com `campaign_id NULL` — o código só grava atribuição quando há algum UTM real), confirmando ausência total de atribuição falsa. | — |
