-- ============================================
-- VERIFICAR STATUS DA IMPLEMENTAÇÃO 2FA (CORRIGIDO)
-- ============================================
...
-- 3. Verificar estrutura da tabela email_templates (se existir)
SELECT 
    CASE 
        WHEN EXISTS (
        ...
        ELSE 'Tabela email_templates NÃO EXISTE'
    END as status_email_templates;

-- 4. Se email_templates existir, mostrar estrutura
SELECT 
    column_name, 
    data_type, 
    is_nullable
...
-- 6. Verificar configurações 2FA do sistema
SELECT 
    setting_key,
    setting_value,
    description,
...
-- 7. Verificar templates de email (usando estrutura real)
SELECT 
    id,
    name,
    subject,
...
SELECT 
    ...
    CASE 
        WHEN EXISTS (SELECT 1 FROM email_templates WHERE name LIKE '%2fa%' OR name LIKE '%verificacao%' OR name LIKE '%codigo%') 
        THEN '✅ Template 2FA EXISTE'
        ELSE '❌ Template 2FA FALTANDO'
