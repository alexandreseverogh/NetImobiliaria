-- Script de Diagnóstico SIMPLIFICADO: Por que Receitas de Destaques não aparece no menu
-- Versão que funciona mesmo se a coluna is_active não existir em role_permissions
...
SELECT 
    ...
    CASE 
        WHEN EXISTS (
            ...
            WHERE table_name = 'user_role_assignments' AND column_name = 'is_active'
        ) THEN (SELECT is_active FROM user_role_assignments WHERE id = ura.id)
        ELSE true
    END as ura_ativo,
    ur.name as role_name,
    ur.is_active as role_ativa,
    CASE 
        WHEN (
            NOT EXISTS (
                SELECT 1 FROM information_schema.columns 
                WHERE table_name = 'user_role_assignments' AND column_name = 'is_active'
            ) OR (SELECT is_active FROM user_role_assignments WHERE id = ura.id) = true
        ) AND ur.is_active = true THEN '✅ OK'
        WHEN EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'user_role_assignments' AND column_name = 'is_active'
        ) AND (SELECT is_active FROM user_role_assignments WHERE id = ura.id) = false THEN '❌ USER_ROLE_ASSIGNMENT INATIVO'
...
-- ============================================
-- Primeiro verificar se coluna existe
DO $$
DECLARE
    v_has_ura_is_active BOOLEAN;
BEGIN
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'user_role_assignments' AND column_name = 'is_active'
    ) INTO v_has_ura_is_active;
    
    IF v_has_ura_is_active THEN
        -- Se coluna existe, usar query com verificação
        PERFORM 1 FROM (
            SELECT 
                sf.id,
                sf.name,
    ...
    ELSE
        -- Se coluna não existe, usar query sem verificação
        PERFORM 1 FROM (
            SELECT 
                sf.id,
                sf.name,
