-- Migration: Criar funcionalidade ValorDestaques (tipo EXECUTE)
-- Data: 2025-01-XX
-- Descrição: Cria funcionalidade ValorDestaques do tipo EXECUTE em /admin/valoresdestaque
...
BEGIN
    ...
    -- Criar funcionalidade ValorDestaques
    ...
    VALUES (
        'ValorDestaques',
        'Gerenciar valores de destaque por estado',
        categoria_id,
        '/admin/valoresdestaque',
        'valores-anuncios-destaques',
        'EXECUTE',
        true,
        NOW(),
        NOW()
    ...
    SET slug = 'valores-anuncios-destaques',
        ...
        url = '/admin/valoresdestaque';
    
    RAISE NOTICE 'Funcionalidade ValorDestaques criada com sucesso';
END $$;

-- Criar permissão EXECUTE para a funcionalidade ValorDestaques
DO $$
DECLARE
    feature_id INTEGER;
    permission_id INTEGER;
BEGIN
    -- Buscar ID da funcionalidade ValorDestaques
    SELECT id INTO feature_id 
    FROM system_features 
    WHERE name = 'ValorDestaques' 
    LIMIT 1;
...
BEGIN
    ...
    IF feature_id IS NOT NULL THEN
        ...
        VALUES ('/admin/valoresdestaque', 'GET', feature_id, 'EXECUTE', true, true)
        ...
        -- Rota da API GET
        INSERT INTO route_permissions_config (route_pattern, method, feature_id, default_action, requires_auth, is_active)
        VALUES ('/api/admin/valoresdestaque', 'GET', feature_id, 'EXECUTE', true, true)
        ON CONFLICT (route_pattern, method) DO UPDATE 
        SET feature_id = EXCLUDED.feature_id, 
            default_action = EXCLUDED.default_action;
        ...
        -- Rota da API PUT
        INSERT INTO route_permissions_config (route_pattern, method, feature_id, default_action, requires_auth, is_active)
        VALUES ('/api/admin/valoresdestaque', 'PUT', feature_id, 'EXECUTE', true, true)
        ON CONFLICT (route_pattern, method) DO UPDATE 
        SET feature_id = EXCLUDED.feature_id, 
            default_action = EXCLUDED.default_action;
