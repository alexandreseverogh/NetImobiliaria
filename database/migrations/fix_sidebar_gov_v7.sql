-- [GOVERNANÇA SIDEBAR V7] - Reconhecimento de Super Admin Global
-- Melhora a detecção de Super Admin para incluir usuários com papéis globais (fora de UTM)
...
DECLARE
    ...
    _res JSONB;
BEGIN
    -- 1. Verificação de Super Admin (Metadados Dinâmicos + Globais)
    SELECT EXISTS (
        -- Verifica se o usuário tem nível Master via Atribuições Globais
        SELECT 1 FROM public.user_role_assignments ura
        JOIN public.user_roles r ON ura.role_id = r.id
        WHERE ura.user_id = p_user_id 
          AND (r.level >= 6 OR r.is_system_role = true)
    ) OR EXISTS (
        -- Verifica se o usuário tem nível Master via Vínculo com Empresa
        SELECT 1 FROM public.user_tenant_membership utm
        JOIN public.user_roles r ON utm.role_id = r.id
        WHERE utm.user_id = p_user_id 
          AND (r.level >= 6 OR r.is_system_role = true)
    ) OR (p_user_id = 'cc8220f7-a3fd-40ed-8dbd-a22539328083'::uuid) -- Hardcoded master rescue
    INTO _v_is_superadm;
