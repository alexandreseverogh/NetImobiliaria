-- ============================================================
-- UPGRADE FINAL: HIERARQUIA DE FERRO E ORDEM DE FÁBRICA
-- ============================================================
-- Esta função agora valida a cadeia: Segmento -> Módulo -> Feature
-- e garante que a ordem definida no Master seja respeitada.
-- ============================================================
...
BEGIN
    ...
    -- [3] Gerar Menu com Trava Hierárquica (Segmento -> Módulo -> Feature)
    ...
    FROM (
        ...
        authorized_items AS (
            ...
            WHERE s.is_active = true
              ...
              AND (
                -- [A] Super Admin (Acesso Ilimitado)
                _v_is_superadm = true
                
                OR (
                    -- [B] TRAVA DE HIERARQUIA: A feature deve estar no Segmento via Módulo
                    EXISTS (
                        SELECT 1 
                        FROM public.system_features f
                        JOIN public.system_feature_modules fm ON f.id = fm.feature_id
                        JOIN public.system_segment_modules sm ON fm.module_id = sm.module_id
                        WHERE f.slug = s.permission_required
                          AND sm.segment_id = _v_segment_id
                          AND sm.is_active = true
                    )
                    AND (
                        -- [C] Validação de Permissões Dentro da Hierarquia
                        s.permission_required IS NULL
                        OR (
                            _v_role_level >= 5 
                            AND EXISTS (
                                SELECT 1 FROM public.system_features f 
                                WHERE f.slug = s.permission_required AND f.is_default_tenant_admin_feature = true
                            )
                        )
                        OR EXISTS (
                ...
                -- [D] Exceções: Overrides específicos de Empresa ainda prevalecem (Caminho de Emergência)
                OR EXISTS (
        ...
        -- [4] ORDEM DE FÁBRICA: Respeita o order_index absoluto do Master
        ...
        mt.order_index
        FROM menu_tree mt
        -- Ordenação prioritária por hierarquia e depois pelo índice master
