-- ESTRATÉGIA HÍBRIDA CORRIGIDA PARA FICHA COMPLETA DO IMÓVEL
...
-- =====================================================
-- 2. VIEW MATERIALIZADA PARA AMENIDADES DETALHADAS (CORRIGIDA)
-- =====================================================
...
SELECT 
    i.id as imovel_id,
    i.codigo,
    -- Amenidades agrupadas por categoria (sem aninhamento de agregações)
    COALESCE(
        json_object_agg(
            ca.nome,
            amenidades_categoria.amenidades
...
LEFT JOIN LATERAL (
    SELECT json_agg(
        json_build_object(
            ...
            'popular', a2.popular,
            'ordem', a2.ordem
        ) ORDER BY a2.ordem, a2.nome
    ) as amenidades
    FROM amenidades a2
    JOIN imovel_amenidades ia2 ON a2.id = ia2.amenidade_id
    WHERE ia2.imovel_id = i.id AND a2.categoria_id = ca.id AND a2.ativo = true
) amenidades_categoria ON true
WHERE i.ativo = true
GROUP BY i.id, i.codigo;

-- =====================================================
-- 3. VIEW MATERIALIZADA PARA PROXIMIDADES DETALHADAS (CORRIGIDA)
-- =====================================================
...
SELECT 
    i.id as imovel_id,
    i.codigo,
    -- Proximidades agrupadas por categoria (sem aninhamento de agregações)
    COALESCE(
        json_object_agg(
            cp.nome,
            proximidades_categoria.proximidades
...
LEFT JOIN LATERAL (
    SELECT json_agg(
        json_build_object(
            ...
            'icone', p2.icone,
            'distancia_metros', ip2.distancia_metros,
            'tempo_caminhada', ip2.tempo_caminhada,
            'observacoes', ip2.observacoes,
            'popular', p2.popular,
            'ordem', p2.ordem
        ) ORDER BY p2.ordem, p2.nome
    ) as proximidades
    FROM proximidades p2
    JOIN imovel_proximidades ip2 ON p2.id = ip2.proximidade_id
    WHERE ip2.imovel_id = i.id AND p2.categoria_id = cp.id AND p2.ativo = true
) proximidades_categoria ON true
WHERE i.ativo = true
GROUP BY i.id, i.codigo;
