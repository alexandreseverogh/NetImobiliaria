...
BEGIN
    ...
    FROM (
        ...
        parent_categories AS (
            ...
            WHERE c.id IN (SELECT category_id FROM authorized_features)
        ),
        all_items AS (
            ...
            UNION ALL
            SELECT 'item_master' as id, 'cat_master' as parent_id, 'Provisionamento' as name, '/admin/master/provisioning' as path, 'ShieldCheckIcon' as icon, 99 as order_index WHERE _v_is_global_master = true
            UNION ALL
            SELECT 'cat_master' as id, NULL as parent_id, 'Master Governance' as name, NULL as path, 'ShieldCheckIcon' as icon, 99 as order_index WHERE _v_is_global_master = true
        )
...
`;
  ...
  try {
    ...
    console.log('Sidebar corrigida com IDs estritamente únicos!');
  } catch (err) {
    console.error(err);
  } finally {
    await client.end();
