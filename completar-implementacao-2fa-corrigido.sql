-- ============================================
-- COMPLETAR IMPLEMENTAÇÃO DO 2FA (CORRIGIDO)
-- ============================================
...
-- 2. Verificar estrutura da tabela email_templates
SELECT 
    column_name, 
    data_type, 
    is_nullable
...
-- 3. Criar template de email 2FA (usando estrutura real)
-- Primeiro, vamos ver qual é a estrutura real da tabela
DO $$
BEGIN
    -- Verificar se já existe template 2FA
    IF NOT EXISTS (
        SELECT 1 FROM email_templates WHERE name = '2fa-code' OR name LIKE '%2fa%'
    ) THEN
        -- Inserir template usando a estrutura real (vamos descobrir os campos corretos)
...
Este é um email automático, não responda.',
    ...
    ELSE
        RAISE NOTICE 'Template 2FA já existe';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Erro ao criar template: %. Estrutura da tabela pode ser diferente.', SQLERRM;
END $$;

-- 4. Verificar estrutura final da tabela users
SELECT 
    column_name, 
    data_type, 
    is_nullable,
...
WHERE name = '2fa-code' OR name LIKE '%2fa%';
