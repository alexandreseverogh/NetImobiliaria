...
BEGIN
    ...
    WITH permitted_features AS (
        ...
        WHERE sf.is_active = true
          ...
          -- FILTRO B: PROVISÃO DA EMPRESA (Obriga ter o módulo E a feature provisionada/default)
          AND (
            v_is_master = true OR
            p_tenant_id IS NULL OR
            (
                -- Condição 1: O Módulo da categoria PRECISA estar habilitado (ou a categoria não pertencer a módulo)
                (
                    EXISTS (
                        SELECT 1 FROM public.system_categorias sc
                        WHERE sc.id = sf.category_id AND sc.module_id IS NULL
                    )
                    OR
                    EXISTS (
                        SELECT 1 FROM public.system_categorias sc
                ...
                AND
                -- Condição 2: A funcionalidade em si PRECISA ser default ou estar nas exceções provisionadas
                (
                    COALESCE(sf.is_default_tenant_admin_feature, false) = true
                    OR
                    EXISTS (
